...
- Verify component-level compliance before reporting complete:
  ...
  - If the design requires scope enforcement (blocking file edits outside allowed patterns), this must be implemented as a custom `BaseTool` subclass with `resolve_permission()` or via the `approval_callback` mechanism — **never** as middleware. Middleware cannot intercept individual tool calls. **`enabled_tools` does NOT enforce file scope** — it only controls which tools the LLM can call, not which paths those tools can access. File-level scope requires per-tool allowlist/permission overrides in the agent profile, validation logic inside the phase-advance tool, or `resolve_permission()` on the tool. **Scope validation in the phase-advance tool must union all three diff sources:** `git diff --name-only origin/main...HEAD` (committed), `git diff --name-only --cached` (staged), and `git diff --name-only` (unstaged). Using only the three-dot syntax misses uncommitted edits.
  ...
  - **Agents are not configured in config.toml.** No `[agents.X]` section exists. Agents are discovered from `.toml` files in agent directories. `~/.vibe/agents/` is the default — no `agent_paths` entry needed. `~/.vibe/skills/` is the default skills directory — no `enabled_skills` entry needed.
  ...
  - **Custom system prompt files go in `~/.vibe/prompts/`, not in source.** `system_prompt_id` resolves `~/.vibe/prompts/<id>.md` before the built-in enum. No source modification needed.
  - **Phase gates that rely solely on hooks are inherently limited.** A hook can inject a message (soft) or exit 2 for retry (but max 3 retries per turn). A hook cannot reliably parse an implementation plan and verify files exist on disk. If a phase has no tangible artifact verify (e.g., "files from IMPLEMENTATION_PLAN.md exist"), the phase is entirely unenforced. Prefer tool-triggered gates with `switch_agent_callback` for enforceable phase transitions.
