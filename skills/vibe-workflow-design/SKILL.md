...
- **Agents are not configured in config.toml.** There is no `[agents.X]` section in `VibeConfig`. Agents are discovered from `.toml` files in agent directories. `~/.vibe/agents/` is the default global agent directory — no `agent_paths` config entry is needed. Similarly, `~/.vibe/skills/` is the default global skills directory — no `enabled_skills` entry is needed to activate skills placed there. Only `enable_experimental_hooks = true` is required in config.toml for hook-based workflows. Adding `agent_paths` or `enabled_skills` entries is unnecessary noise.
- **Custom system prompt files go in `~/.vibe/prompts/`, not in source.** The `system_prompt_id` key in an agent profile TOML resolves by checking `~/.vibe/prompts/<id>.md` first, then the built-in `SystemPrompt` enum. Dropping a `.md` file in `~/.vibe/prompts/` is sufficient — no source modification to `core/prompts/` is needed. This is Tier A, not Tier D.
...
- **All phase profiles must be usable as primary agents.** `AgentManager.__init__` rejects any profile with `agent_type != AgentType.AGENT` as the initial agent. Every custom agent profile used in a workflow must either set `agent_type = "agent"` explicitly or omit it (since `"agent"` is the default). Subagent-typed profiles (`agent_type = "subagent"`) cannot be used as standalone phase profiles.
- **`enabled_agents` is an allowlist, not an addlist.** If `enabled_agents` is set in config, `available_agents` returns only agents matching that list — built-in agents like `default`, `plan`, `accept-edits` disappear unless explicitly included. When adding custom agents to `enabled_agents`, you must also list every built-in agent you still want available.
...
- **If a phase requires explicit user approval, the profile must include `ask_user_question`, or the transition must be triggered by the user's next message.** The agent cannot "block and wait" — it can only present text and stop. Without `ask_user_question` in the profile's `enabled_tools`, the approval decision must be an asynchronous user reply, and the next phase's tool must only be callable after that reply.
- **Scope validation in phase-advance tools must union all three diff sources.** `git diff --name-only origin/main...HEAD` only catches committed changes. Staged and unstaged changes require `git diff --name-only --cached` and `git diff --name-only` respectively. A phase-advance tool that validates file scope must union all three, or uncommitted edits will be invisible to the check.
- **If a phase needs to read structured state, ensure the profile includes a file-reading tool.** Agent profiles that omit `read_file` cannot read JSON state files — `bash` + `cat` works for raw text but is fragile for structured parsing. Include `read_file` (or a custom tool) in any profile that needs to read state files.
