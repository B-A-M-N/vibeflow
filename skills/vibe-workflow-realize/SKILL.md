...
- **Skill-bundled tools work but verify they load.** `skill_dir/tools/` and `config.tool_paths` both work for tool discovery (recursive `rglob`), but always confirm the tool appears in the schema — check that the class inherits `BaseTool` correctly and has no import errors before relying on it.
...
- **Phase switching preserves middleware state.** `switch_agent` calls `reload_with_initial_messages(reset_middleware=False)` — middleware state (e.g., `AutoCompactMiddleware` token counters) accumulates across the full session. Design phase transitions accordingly.
- **Agent TOML must use flat top-level keys.** No `[agent]`, `[agent.system_prompt]`, `[agent.permissions]` sections — `from_toml` won't parse them. Only `display_name`, `description`, `safety`, `agent_type` are metadata. Everything else is flat keys or `[tools.X]` sub-tables.
- **Skills must be SKILL.md markdown files, not Python constants.** `SkillInfo` is internal — the correct artifact is always a directory with a `SKILL.md` and YAML frontmatter.
- **Agents are not configured in config.toml.** No `[agents.X]` section. `~/.vibe/agents/` is the default agent directory; `~/.vibe/skills/` is the default skills directory. No `agent_paths` or `enabled_skills` config entries needed.
- **All phase profiles must set `agent_type = "agent"` (or omit it).** `AgentManager.__init__` rejects non-agent profiles as the initial agent. Every custom profile used as a phase agent must be agent-typed.
- **`enabled_agents` is an allowlist, not an addlist.** Setting it excludes all built-in agents not explicitly listed. Include every built-in agent you still want available.
- **Custom system prompt files go in `~/.vibe/prompts/`, not in source.** `system_prompt_id` resolves `~/.vibe/prompts/<id>.md` before the built-in enum. Dropping a `.md` file there is sufficient — no source modification to `core/prompts/`.
- **Middleware "blocking" is overstated.** `STOP` kills the session; `INJECT_MESSAGE` can be ignored. For actual phase-gate enforcement, use tool-triggered profile switching with `enabled_tools`/`disabled_tools`, not middleware.
- **Phase gates that rely solely on hooks are inherently limited.** A hook cannot reliably parse an implementation plan and verify files exist. If a phase has no tangible artifact verify, it is entirely unenforced. Prefer tool-triggered gates with `switch_agent_callback`.
- **If a phase requires explicit user approval, the profile must include `ask_user_question`, or the transition must be triggered by the user's next message.** The agent cannot block and wait. Without `ask_user_question` in the profile, the approval must be an asynchronous user reply.
- **Scope validation in phase-advance tools must union all three diff sources:** committed (`git diff --name-only origin/main...HEAD`), staged (`git diff --name-only --cached`), and unstaged (`git diff --name-only`). Using only the three-dot syntax misses uncommitted edits.
- **If a phase needs to read structured state, ensure the profile includes a file-reading tool.** Profiles that omit `read_file` cannot read JSON state files — `bash` + `cat` works for raw text but is fragile for structured parsing.
