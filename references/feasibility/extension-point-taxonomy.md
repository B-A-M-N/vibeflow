...
- Skill format: YAML frontmatter (`name`, `description`, `allowed-tools`, `compatibility`, `user-invocable`) + markdown body. `allowed-tools` / `allowed_tools` is **advisory only** — it is never used to filter `ToolManager.available_tools`. The model sees all available tools regardless. Use agent profile `overrides.enabled_tools` for actual restriction. Full field constraints: `name` 1–64 chars matching `^[a-z0-9]+(-[a-z0-9]+)*$`; `description` 1–1024 chars; `compatibility` max 500 chars; `allowed_tools` space-delimited string or list; `user_invocable` controls slash command menu visibility. Skill name must match directory name (mismatch warns but doesn't fail). Skills load once per invocation — re-invoke to reload. Trust-gated discovery: `.vibe/skills/` and `.agents/skills/` require trusted project directories; `~/.vibe/skills/` always loads. First-wins dedup: custom skills cannot override builtins with the same name.
...
| `vibe/cli/textual_ui/` | Textual TUI: `VibeApp` (main app), `EventHandler` (event-to-widget mapping), `Banner` + `PetitChat` (braille-art banner widget). Modify for custom UI behavior, banner replacement, or new event consumers. See `event-system-reference.md` for TUI surface details. |
...
- **HARD INVARIANT: Source modifications MUST be verified by constructed tests.** Any change to `vibe/core/` source files (AgentLoop, middleware, tools, events, session, hooks, etc.) is incomplete until tests have been constructed and run that verify the modification works as intended. This is not optional and not advisory — it is a gating requirement. A source change without tests is a broken change. See Pattern 21 for the verification pattern. At minimum, the test suite must: (a) exercise the modified code path, (b) assert the expected behavior, (c) fail if the modification is reverted or broken, and (d) be runnable independently of the full Vibe runtime (unit tests, mock-based integration tests, or targeted script-level verification).
