...
- Verify component-level compliance before reporting complete:
  ...
  - Custom tools must be skill-bundled or in config/tool_paths — **never** in `vibe/core/tools/builtins/`. That directory is core infrastructure; placing workflow-specific tools there is a Tier D source modification. Skill-bundled tool discovery works via `skill_dir/tools/` and `config.tool_paths`, but always verify the tool actually loads — check the tool schema to confirm it appears before relying on it.
