...
- Preserve the design decision contract. Implement selected surfaces and record implementation evidence; do not implement rejected or not-applicable surfaces unless a scoped design deviation is recorded first.
...
- required top-level sections: `name`, `goal`, `phases`, `tooling`, `state`, `middleware`, `approval_gates`, `evidence`, `failure_policy`, `commands`, and `validation`
- `validation.serial: true`, `validation.evidenceRequired: true`, and `validation.mutatesWorkflow: false`
...
Also run or prepare `design-contract-linter.py`. Selected surfaces must have implementation evidence by the end of apply; rejected and not-applicable surfaces must not create required implementation work.
